...
% front (0 through 63) or the back (64 through 127). The next letter indicates 
...
run() ->
    Data = common:readlines("inputs/day5.txt"),
    AllSeats = find_seats(Data),
    common:print_day(1, common:maximum(0, AllSeats)),
    common:print_day(2, hd(lists:seq(28, 842)-- lists:sort(AllSeats))).
...
find_seats([], Acc) ->
    Acc;
find_seats([H|T], Acc) ->
    find_seats(T, [find_seat(H)|Acc]).

find_seat(Bin) when is_binary(Bin) ->
    find_seat(binary_to_list(Bin));
find_seat(Seq=[_, _, _, _, _, _, _, _, _, _]) ->
    [S1,S2,S3,S4,S5,S6,S7,S8,S9,S10]=
        replace($F, 0, replace($B, 1, replace($R, 1, replace($L, 0, Seq)))),
    <<Row:7>> = <<S1:1, S2:1, S3:1, S4:1, S5:1, S6:1, S7:1>>,
    <<Column:3>> = <<S8:1, S9:1, S10:1>>,
    (8 * Row) + Column.
    
replace(Elm, NewElm, List) ->
    replace(Elm, NewElm, List, []).
replace(_,_,[],Acc)->
    lists:reverse(Acc);
replace(Elm, NewElm, [Elm|T], Acc) ->
    replace(Elm, NewElm, T, [NewElm|Acc]);
replace(Elm, NewElm, [Other|T], Acc) ->
    replace(Elm, NewElm, T, [Other|Acc]).
